...
//   - filtro de sensibilidad idéntico al de la vista:
//       NORMAL             → solo view
//       SENSITIVE_CLINICAL → view + sensitive_clinical_view
//       SENSITIVE_FINANCIAL → view + financial_view
//       SENSITIVE_PERSONAL  → view + sensitive_personal_view
 ...
 * NUNCA se sanitiza para URL — solo para el valor del header.
 */
...
export async function downloadPatientDocument(
  ...
  if (!can(ctx.permissions, "patient_documents.download") || !can(ctx.permissions, "patient_documents.view")) {
  ...
  if (!canSeeDocumentSensitivity(ctx.permissions, doc.sensitivityLevel)) {
    ...
    return fail("FORBIDDEN", "No tiene permiso para ver documentos con este nivel de sensibilidad.");
  }
  ...
  if (!storageResult.ok) {
    await run(async (exec) => {
      await recordAudit(exec, {
        ...
        entityType: "patient",
        entityId: patientId,
        // reason = storageResult.reason es seguro (BLOCKED/NOT_FOUND). Detail NO se incluye:
        // puede mencionar el endpoint, el bucket o el storageKey en mensajes internos.
